...
int main() {
    pt p[4], x, t[4][3];
    while(scanf("%lf%lf%lf", &p[0].x, &p[0].y, &p[0].z) != EOF)
    {
        ...
        {
            int k = 0;
            for(int j=0;j<4;j++) if(i != j) t[i][k++] = p[j];
        }
        for(int i=0;i<4;i++)
        {
            int ok = 1;
            pt c = (t[i][0]+t[i][1]+t[i][2])/3;
            for(int j=0;j<4;j++) if(i != j) if(seg_tri_intersect(x, c, t[j])) ok = 0;
            if(ok) cout << "S";
            else cout << "N";
        }
        cout << endl;
